...
/**
 * Created by Pavel on 13.11.2015.
 */
public class NMEA {
    ...
    class GPRMC implements SentenceParser {
        public boolean parse(String[] tokens, GPSPosition position) {
            ...
            try {
                position.date = simpleDateFormat.parse(tokens[9]);
    ...
    public class GPSPosition {
        ...
        public String toString() {
            return String.format("POSITION: lat: %f, lon: %f, time: %f, Q: %d, dir: %f, alt: %f, vel: %f, date: %s",
                    lat, lon, time, quality, dir, altitude, velocity, simpleDateFormat.format(date));
        }
    }
